...
class nggVoting {
	...
	function __construct() {
		...
		//NGG >= 2.0.57
		add_filter('ngg_manage_gallery_fields', array(&$this, 'addGalleryVotingOptionsv2'), 11, 2);
		...
		*/
		...
		//add_action('load_nextgen_gallery_modules', array(&$this, 'nggLoadModules'));
		add_filter('ngg_displayed_gallery_cache_params', array(&$this, 'set_voting_cache_params'));
	...
	// API Functions {
		...
		//remove NGG's caching, as we are adding user's dynamic content to the gallery templates
		//deprecated
		function nggLoadModules($registry) {
			if(class_exists('C_Photocrati_Cache')) {
				C_Photocrati_Cache::$enabled = false;
			}
		}
		
		//invalidate the cache every mirco second. Too many variables about serving the cache to try worry about it for now
		function set_voting_cache_params($arr) {
			$arr['nggvoting_time'] = microtime(); //will invalidate the cache every request
  ...
  // }
	...
	// Admin Functions {
			...
			<div class="wrap">
				...
				<?php if($list && !class_exists('nggVotingPremium')) { ?>
					<div class="updated below-h2">
						Look at the people voting for your images! Have you returned the favour by <a target="_blank" href="http://wordpress.org/support/view/plugin-reviews/nextgen-gallery-voting">rating NGG Voting</a>?<br />
		...
		function addImageVoteColCount($numCols) {
			$cnt = $numCols+1;
			
			add_filter('ngg_manage_images_column_'.$cnt.'_header', array(&$this, 'addImageVoteColHeader'));
			add_filter('ngg_manage_images_column_'.$cnt.'_content', array(&$this, 'addImageVoteColContent'), 10, 2);
			
			return $cnt;
		}
		
		function addImageVoteColHeader($header) {
			return 'Voting Options';
		}
		
		function addImageVoteColContent($output='', $picture=array()) {
			ob_start();
			$this->addImageVotingOptions('nggv_image_vote_options', $picture->pid);
		...
		/**
		 * Add the voting options to each image
		 ...
		 * Add the voting options to the gallery
		...
		function addGalleryVotingOptionsv2($fields=array(), $gallery=NULL) {
			$fields['left']['ngg_voting_options'] = array(
					'callback'	=>	array(&$this, 'ngg_voting_options'),
					'label'		=>	_('Voting Options:'),
					'tooltip'   =>  NULL,
					'id'		=>	'ngg_voting_options'
				);
			
			return $fields;
		}
		
		function ngg_voting_options($gallery=array()) {
			$this->addGalleryVotingOptions($gallery->gid);
		}
